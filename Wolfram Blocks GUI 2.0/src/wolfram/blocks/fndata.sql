...
CREATE TABLE IF NOT EXISTS FUNCTION.FUNCTIONS (
...
CREATE TABLE IF NOT EXISTS FUNCTION.INPUTS (
	IN_ID INT NOT NULL,
	FN_NAME VARCHAR(100) REFERENCES FUNCTION.FUNCTIONS(NAME),
	LABEL VARCHAR(100),
	CONSTRAINT PK_INPUT PRIMARY KEY (IN_ID, FN_NAME)
);

#LUT for input attributes
CREATE TABLE IF NOT EXISTS FUNCTION.INPUT_ATTRIBUTES (
	ID INT PRIMARY KEY,
	NAME VARCHAR(250) NOT NULL
);

#Pairs unique input with its attributes
CREATE TABLE IF NOT EXISTS FUNCTION.INPUT_DETAILS (
	IN_ID INT,
	FN_NAME VARCHAR(100),
	ATNAME INT REFERENCES FUNCTION.INPUT_ATTRIBUTES,
	CONSTRAINT FK_INPUT FOREIGN KEY (IN_ID, FN_NAME) REFERENCES FUNCTION.INPUTS(IN_ID, FN_NAME)
);

#LUT for block attributes
CREATE TABLE IF NOT EXISTS FUNCTION.FN_ATTRIBUTES (
	ID INT PRIMARY KEY,
	NAME VARCHAR(100) NOT NULL
);

#pairs each function type with its block attributes
CREATE TABLE IF NOT EXISTS FUNCTION.FN_DETAILS (
	FN_NAME VARCHAR(100) REFERENCES FUNCTION.FUNCTIONS(NAME),
	ATNAME INT REFERENCES FUNCTION.FN_ATTRIBUTES
);

#Populate input attribute LUT
INSERT INTO FUNCTION.INPUT_ATTRIBUTES(ID, NAME)
...
INSERT INTO FUNCTION.FN_ATTRIBUTES(ID, NAME)
VALUES (1, 'TIGHTFIT');
